...
SELECT id, app_name, reviews FROM analytics WHERE rating >= 4.8 ORDER BY reviews DESC LIMIT 1;
...
-- 7. Find the name, price, and rating of the most expensive app with a rating less than 3.
SELECT app_name, price, rating FROM analytics WHERE rating < 3 ORDER BY price DESC LIMIT 1;

-- 8. Find all apps with a min install not exceeding 50, that have a rating, ordered by highest rated first.
...
SELECT id, app_name, reviews FROM analytics WHERE price >= 0.10 AND price <= 1.00 ORDER BY reviews DESC LIMIT 10;

-- 11. Find the most out-of-date app.
SELECT id, app_name, last_updated FROM analytics ORDER BY last_updated ASC LIMIT 1;

-- 12. Find the most expensive app.
SELECT id, app_name, price FROM analytics ORDER BY price DESC LIMIT 1;
